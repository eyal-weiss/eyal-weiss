## Eyal Weiss

<img src="profile.jpg" align="right" width="220">

Postdoctoral Scholar in **AI & Robotics** at the [Computational Robotics Lab (CRL)](https://crl.cs.technion.ac.il/), Computer Science Faculty, [Technion](https://www.cs.technion.ac.il/) — Israel Institute of Technology.

My research focuses on **Planning, Search & Optimization**, with broad applications in AI & robotics. I develop theory, algorithms and planners/solvers.

### Currently Working On
- Motion planning and manipulation for dual-armed robots
- Motion planning for snake-like robots
- Multi-agent warehouse optimization
- Planning with dynamically estimated action models
- Bidirectional heuristic search

### Recent Highlights
- **Best Paper Award** at SoCS 2025 for [From Agent Centric to Obstacle Centric Planning: A Makespan-Optimal Algorithm for the Multi-Agent Warehouse Rearrangement Problem](https://ojs.aaai.org/index.php/SOCS/article/view/35985/38140)
- [PDBs Go Numeric: Pattern-Database Heuristics for Simple Numeric Planning](https://ojs.aaai.org/index.php/AAAI/article/view/34851) — **AAAI 2025**
- [Bidirectional Search while Ensuring Meet-In-The-Middle via Effective and Efficient-to-Compute Termination Conditions](https://ijcai-preprints.s3.us-west-1.amazonaws.com/2025/1923.pdf) — **IJCAI 2025**
- [BLITstar](https://github.com/yi213-robotic/BLITstar-ICRA2025) motion planner integrated into the [Open Motion Planning Library (OMPL)](https://ompl.kavrakilab.org/)
- Workshop organizer: RIPL @ ICAPS 2026, HSDIP @ ICAPS 2025, RDDPS @ ICAPS 2023–2024

### Featured Repositories
- [**wh-rearrangement**](https://github.com/CRL-Technion/wh-rearrangement) — Makespan-optimal multi-agent warehouse rearrangement (SoCS 2025 Best Paper)
- [**plandem-public**](https://github.com/eyal-weiss/plandem-public) — Domain-independent planner with dynamically estimated action models
- [**BLITstar**](https://github.com/yi213-robotic/BLITstar-ICRA2025) — Bidirectional lazy incremental motion planner (integrated into OMPL)
- [**MEET**](https://github.com/yi213-robotic/MEET) — Bidirectional search with meet-in-the-middle guarantees

Outside of research, I am an active investor. I occasionally share opinions on my [X profile](https://x.com/eyal__weiss) and publish investment theses and financial models as GitHub repositories.

### Links

[![Website](https://img.shields.io/badge/Website-eyal--weiss.github.io-blue)](https://eyal-weiss.github.io)
[![Google Scholar](https://img.shields.io/badge/Google_Scholar-Profile-green)](https://scholar.google.com/citations?user=LO5sHMIAAAAJ&hl=en)
[![ORCID](https://img.shields.io/badge/ORCID-0000--0003--0508--6213-brightgreen)](https://orcid.org/0000-0003-0508-6213)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue)](https://www.linkedin.com/in/eyal-weiss-research)
[![YouTube](https://img.shields.io/badge/YouTube-Lectures-red)](https://www.youtube.com/channel/UCWdweT0ooo0buk2m81lW9mw/playlists)
[![X](https://img.shields.io/badge/X-@eyal__weiss-black)](https://x.com/eyal__weiss)
